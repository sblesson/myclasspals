...
const Account = ({ user }) => {
  const userDetails = user.map(userItem => (
    <div className='profile-content-details' key={userItem._id}>
      ...
      <div className='school'>{userItem.schoolName}</div>
      <div>
        <span className='grade'>{userItem.grade}</span>
        <span className='class-room'>{userItem.classRoom}</span>
      </div>

      <div className='child-name'>{userItem.childName}</div>
    </div>
  ));
